...
public:
    ...
    std::map<std::string, std::shared_ptr<asio::ip::tcp::socket>>
        channelSockets;
    void stop();
    ...
    EventQueue<std::pair<std::unique_ptr<asio::streambuf>, std::string>>
        eventQueue;
    ...
    // int buyItem(const std::string& channel, const std::string& username,
    // std::string& what, unsigned long long howmuch);
    // int sellItem(const std::string& channel, const std::string& username,
    // std::string& what, unsigned long long howmuch);
...
private:
    ...
    std::chrono::high_resolution_clock::time_point currentTrigger =
